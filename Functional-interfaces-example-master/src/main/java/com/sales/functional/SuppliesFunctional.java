...
public class SuppliesFunctional {
    ...
    public static void loadMenu(){
        ...
        switch(op){
            ...
            case "5":
                ...
                break;
            case "6":
                System.out.print("Ventas con monto total de venta: ");
                getTotalAmount().forEach(System.out::println);
                break;
            case "7":
                System.out.print("Clientes femeninas que compraron In store: ");
                getFemaleCustomerAndInstore();
                break;
            case "8":
                System.out.print("Numero de productos comprados segmentados por tags: ");
                getNumberOfProducts();
                break;
            case "9":
                System.out.print("Cuántos hombres y mujeres han usado cuponese: ");
                getCouponUses();
                break;
            case "10":
                System.out.print("Máxima compra y mínima compra: ");
                getMostExpensiveAndCheapestPurchase();
                break;
            default:
    ...
    public static void getOnlinePurchases(){
        ...
        ArrayList<Sale> result = sales.stream().filter(onlinePurchased)
    ...
    public static void salesByYear(String inYear){
        Function<Sale,String> getYear =
                sale -> String.valueOf(sale.getSaleDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear());
    ...
    public static void salesBySatisfaction(String inSatis){
        Consumer<String> satisfaction = satis -> sales.stream().filter(
                sale -> sale.getCustomer().getSatisfaction().toString().equals(satis)).collect(Collectors.toCollection(ArrayList::new)).forEach(System.out::println);
        satisfaction.accept(inSatis);
    }

    /** 6. Calcule el monto total que pagó el cliente en cada venta.     */
    public static ArrayList<Sale> getTotalAmount(){
        ArrayList<Sale> result = sales.stream()
                .map(sale -> {
                    ...
                    sale.setTotal(tempTotal);
                    return sale;
                })
                .collect(Collectors.toCollection(ArrayList::new));
        //Todo hacerlo por medio de reduce

        return result;
    }

    /** 7. Obtenga todas las ventas en las que el comprador es una mujer y fue comprado en la tienda ('in store')*/
    public static void getFemaleCustomerAndInstore(){
        ArrayList<Sale> result = sales.stream()
                .filter(sale -> sale.getCustomer().getGender().equals("F")
                        && sale.getPurchasedMethod().equals("In store"))
                .collect(Collectors.toCollection(ArrayList::new));
        result.forEach(System.out::println);
    }

    /** 8. Obtenga el número de productos comprados por todos los clientes segmentándolos por etiquetas(tags) */
    public static void getNumberOfProducts(){
        /** Calculando el total de items pero sin segmentar por tags*/
//        sales.stream()
//                .forEach(sale -> sale.setTotal((double) sale.getItems().stream().count()));
//        ArrayList<Sale> result = sales.stream()
//                .collect(Collectors.toCollection());
//        result.forEach(System.out::println);
        /** Aplanando las compras*/
        Map<String, Long> conteoPorEtiqueta = sales.stream()
                .flatMap(sale -> sale.getItems().stream())
                .flatMap(product -> product.getTags().stream())
                .collect(Collectors.groupingBy(tag -> tag, Collectors.counting()));

        for (Map.Entry<String, Long> entry : conteoPorEtiqueta.entrySet()) {
            String tag = entry.getKey();
            long count = entry.getValue();
            System.out.println("Etiqueta: " + tag + ", Número de productos comprados: " + count);
        }
    ...
    /** 9. Obtenga cuantos hombres usaron cupón y cuantas mujeres usaron cupón; */
    public static void getCouponUses(){
        System.out.println(sales.stream().filter(sale -> sale.getCouponUsed()
                && sale.getCustomer().getGender().equals("M")).count()+ " hombres han usado cupones.");
        System.out.println(sales.stream().filter(sale -> sale.getCouponUsed()
                && sale.getCustomer().getGender().equals("F")).count()+ " mujeres han usado cupones.");

    }

    /** 10. Obtenga la venta con la compra más costosa y la venta con la compra más barata*/

    public static void getMostExpensiveAndCheapestPurchase(){
        List<Sale> sales = getTotalAmount();
        Optional<Sale> min;
        min = sales.stream().min((a, b) -> (int) (a.getTotal() -b.getTotal()));
        Optional<Sale> max;
        max = sales.stream().max((a, b) -> (int) (a.getTotal() -b.getTotal()));
        //        max = sales.stream().max((a, b) -> (a.getTotal().compareTo(b.getTotal())));
        //        max = sales.stream().max(Comparator.comparing(Sale::getTotal));
        System.out.println("La compra más grande fue de: "+max.get().getTotal());
        System.out.println("La compra más pequeña fue de: "+min.get().getTotal());
    }
}
